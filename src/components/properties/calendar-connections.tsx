...
import { Button, DetailSection, Input, Label, Select, StatusBadge } from "@/components/ui";
...
function mapCalendarStatus(status: string) {
  if (status === "synced") return "connected" as const;
...
export function CalendarConnections({ property }: { property: PropertyCalendar }) {
  ...
  return (
    <DetailSection
      title="Calendar & booking source"
      description="Connect an ICS feed or note the booking source. Phase 5 will use this to generate turnover jobs automatically."
      actions={
        <StatusBadge status={mapCalendarStatus(property.calendarStatus)}>
    ...
    >
      <dl className="space-y-1 text-sm">
        <div className="flex justify-between gap-3">
          ...
          <dd className="font-medium text-[var(--text-primary)]">{property.bookingSource}</dd>
        </div>
        <div className="flex justify-between gap-3">
          <dt className="text-[var(--text-secondary)]">ICS URL</dt>
      ...
      {property.bookings.length > 0 ? (
        <div className="mt-3 space-y-1">
          ...
          {property.bookings.slice(0, 3).map((b) => (
            <p key={b.id} className="text-xs text-[var(--text-secondary)]">
      ...
      <form
        className="mt-4 space-y-3 rounded-[var(--radius-md)] border border-[var(--border)] p-3"
        action={(fd) => {
          setError(null);
